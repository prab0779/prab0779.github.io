...
export const TradeCalculator: React.FC<TradeCalculatorProps> = ({ items }) => {
  ...
  const StatusPill = () => {
    const tone =
      tradeLabel === "FAIR"
        ? { bg: "bg-black/35", border: "border-gold/35", text: "text-gold-bright", dot: "bg-[var(--gold-bright)]" }
        : tradeLabel === "WIN"
        ? { bg: "bg-emerald-900/15", border: "border-emerald-700/30", text: "text-emerald-200", dot: "bg-emerald-400" }
        : tradeLabel === "LOSE"
        ? { bg: "bg-red-900/15", border: "border-red-700/30", text: "text-red-200", dot: "bg-red-400" }
        : { bg: "bg-black/25", border: "border-zinc-700/30", text: "text-zinc-300", dot: "bg-zinc-400" };

    return (
      <div className={`w-full rounded-xl border px-4 py-3 text-center font-semibold tracking-wide ${tone.bg} ${tone.border}`}>
        <span className={`inline-flex items-center justify-center gap-2 ${tone.text}`}>
          <span className={`inline-block w-2.5 h-2.5 rounded-full ${tone.dot}`} />
          <span className="text-sm sm:text-base">{tradeLabel === "—" ? "NO TRADE" : tradeLabel}</span>
        </span>
      </div>
    );
  };
  ...
  const ItemModal: React.FC<{
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
        <div className="w-full max-w-lg rounded-2xl border border-gold/35 bg-zinc-950 shadow-2xl">
          ...
          <div className="p-5">
            <div className="relative mb-4">
              ...
              <input
                ...
                className="w-full rounded-xl bg-zinc-900/60 border border-zinc-700/60 pl-10 pr-3 py-2 text-white placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-[var(--gold-bright)]"
              />
            </div>
            ...
            <div className="max-h-[360px] overflow-y-auto space-y-2 pr-1">
              {filtered.map((it) => (
                <button
                  key={it.id}
                  onClick={() => onSelect(it)}
                  className="w-full rounded-xl border border-zinc-800/80 bg-zinc-900/40 hover:bg-zinc-900/70 px-3 py-3 flex items-center gap-3 transition"
                >
                  <div className="w-10 h-10 rounded-lg bg-black/40 border border-zinc-700/70 flex items-center justify-center overflow-hidden">
              ...
              {filtered.length === 0 && <div className="text-center text-zinc-400 py-8">No items found.</div>}
            </div>
          </div>
        </div>
      </div>
  ...
  const Side: React.FC<{ side: "sent" | "received" }> = ({ side }) => {
    ...
    const title = side === "sent" ? "You're giving" : "You're getting";
    const subtitle = side === "sent" ? "Left side" : "Right side";

    return (
      <div className="rounded-2xl border border-gold/25 bg-black/28 p-4 sm:p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            onClick={openModal}
            className="rounded-lg border border-gold/45 bg-black/25 hover:bg-black/45 px-3 py-2 text-gold-bright text-sm font-semibold transition"
          >
            Add
          </button>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 sm:gap-4">
          {Array.from({ length: 9 }, (_, idx) => {
            // plus in the middle like screenshot
            if (idx === 4) {
              return (
                <button
                  key={idx}
                  onClick={openModal}
                  className="aspect-square rounded-xl border border-gold/25 bg-black/20 hover:bg-black/35 transition flex items-center justify-center"
                  title="Add item"
                >
            ...
            // map items skipping middle index
            ...
            return (
              <div
                key={idx}
                className="aspect-square rounded-xl border border-gold/20 bg-gradient-to-b from-black/20 to-black/45 hover:border-gold/35 transition relative overflow-hidden"
              >
                {tradeItem ? (
                  <>
                    <button
                      onClick={() => removeItem(mapIndex, side)}
                      className="absolute top-2 right-2 z-10 rounded-md bg-black/55 border border-gold/25 p-1 text-zinc-200 hover:text-white"
                      title="Remove"
                    >
                      <X className="w-4 h-4" />
                    </button>
                      ...
                      <div className="w-full text-center">
                        ...
                        <div className="mt-1 flex items-center justify-center gap-1">
                          <button
                            ...
                            className="w-6 h-6 rounded bg-zinc-900/70 border border-zinc-700/70 text-white hover:bg-zinc-800"
                            disabled={tradeItem.quantity <= 1}
                          >
                            -
                          </button>

                          {/* NO SPINNER: use text + numeric inputMode */}
                          <input
                            type="text"
                            inputMode="numeric"
                            pattern="[0-9]*"
                            ...
                            className="no-arrows w-10 h-6 rounded bg-zinc-900/70 border border-zinc-700/70 text-white text-center text-xs focus:outline-none focus:ring-2 focus:ring-[var(--gold-bright)]"
                          />
                  ...
                  <div className="h-full w-full flex items-center justify-center">
                    <div className="w-[72%] h-[72%] rounded-lg border border-gold/10 bg-black/15" />
                  </div>
                )}
              </div>
            );
        ...
        <div className="mt-5 rounded-xl border border-gold/20 bg-black/30 px-4 py-3">
  ...
  const pctText = hasTrade && maxSpan ? ((diff / maxSpan) * 100).toFixed(1) : "0.0";

  return (
    <div className="max-w-6xl mx-auto px-4 py-10">
      {/* Title (NOT blurry: remove gold-glow, use clean shadow) */}
      <div className="text-center mb-6">
        <h1
          className="text-4xl sm:text-5xl font-extrabold text-gold-bright tracking-tight"
          style={{
            textShadow: "0 3px 12px rgba(0,0,0,0.75)", // cleaner, less blur
          }}
        >
      ...
      {/* Main board (less “white”: softer borders + darker fills) */}
      <div className="rounded-2xl border border-gold/20 bg-black/20 shadow-2xl overflow-hidden">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-0">
          <div className="p-5 sm:p-6 border-b lg:border-b-0 lg:border-r border-gold/15">
            <Side side="sent" />
          </div>
        ...
        <div className="border-t border-gold/15 bg-black/25 p-5 sm:p-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <span
                className={`px-3 py-1 rounded-lg font-bold text-sm border ${
                  diff === 0
                    ? "bg-emerald-900/20 text-emerald-200 border-emerald-700/25"
                    : diff > 0
                    ? "bg-emerald-900/15 text-emerald-200 border-emerald-700/25"
                    : "bg-red-900/15 text-red-200 border-red-700/25"
                }`}
              >
                {diff.toLocaleString()} ({pctText}%)
              </span>
            </div>

            <button
              onClick={resetAll}
              className="inline-flex items-center gap-2 rounded-lg bg-red-900/40 hover:bg-red-900/55 text-white font-semibold px-4 py-2 border border-red-700/25 transition"
            >
              <X className="w-4 h-4" />
              Reset
            </button>
          ...
          {/* Progress bar (muted, not too bright) */}
          <div className="relative h-3 rounded-full overflow-hidden border border-gold/20 bg-zinc-900">
            <div className="absolute inset-0 bg-gradient-to-r from-red-700 via-yellow-600 to-emerald-600 opacity-85" />
            <div className="absolute left-1/2 top-0 h-full w-[2px] bg-black/60" />
            <div
              className="absolute top-1/2 -translate-y-1/2 w-3 h-3 rounded-full border border-black/60 bg-white/75 shadow"
